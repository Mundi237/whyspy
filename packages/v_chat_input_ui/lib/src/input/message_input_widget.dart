...
class _VMessageInputWidgetState extends State<VMessageInputWidget> {
  ...
  void initState() {
    ...
    // Load draft when widget initializes
    _loadDraft();
                            ...
                            MessageTextFiled(
                              ...
                              onSubmit: (v) {
                                if (v.isNotEmpty) {
                                  ...
                                  _deleteDraft(); // Delete draft when message is sent
                                  _text = "";
                                  _currentUrl = null;
                                  _currentUrls = [];
                                  _textEditingController.clear();
    ...
    if (res != null) {
      switch (res) {
        ...
        case AttachEnumRes.location:
          ...
          if (result != null) {
            // Handle the result based on the actual API structure
            // LocationResult has latLng property and formattedAddress
  ...
  void dispose() {
    ...
    _draftSaveTimer?.cancel(); // Cancel any pending draft saves
  ...
  Future<void> _deleteDraft() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.remove(_getDraftKey());
      _draftSaveTimer?.cancel(); // Cancel any pending saves
    } catch (e) {
      // Handle any errors silently
    }
  }
